#!/bin/bash
if [ ! -f "/shared/current_test.txt" ];then
    touch /shared/current_test.txt
fi
...
inotifywait -m -r -e close_write,create,delete,move /var/www/html/wp-content /shared/current_test.txt --format '%w%f %e %T' --timefmt '%Y-%m-%d %H:%M:%S' | while read file event time
do
    if [ "$file" = "/shared/current_test.txt" ];then
